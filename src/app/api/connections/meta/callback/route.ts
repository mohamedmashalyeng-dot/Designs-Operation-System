...
import { completeMetaConnection } from "@/lib/meta/service";

export async function GET(request: NextRequest) {
  const user = await requireUser();
  const redirectBase = new URL("/connections/meta", request.url);
  ...
  if (oauthError) {
    redirectBase.searchParams.set("error", oauthError === "access_denied" ? "access_denied" : oauthError);
